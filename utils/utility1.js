...
function ask_riddle(riddle) {
    const caches = readline.question(`${riddle.taskdescription}`)
    if (riddle.answers !== undefined) {
        console.log(`${riddle.answers}`)
        // const indexer=readline.question("enter the correctanswers index")
        if(indexer===riddle.correctanswer){
            return "good work"
        }
        else{
            console.log("Try again")
...
export function total_time(player) {
    ...
    for (let i = 0; i < times_list.length; i++) {
        seconds_counter += times_list[i]
    }
    return seconds_counter
}
export function avrege(player) {

    const avg = total_time(player) / player.times.length
    return avg
}
